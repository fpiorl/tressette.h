...
#define TRESSETTE_MEMCPY(dest, src, size)                                      \
  __builtin_memcpy((dest), (src), (size))
#define TRESSETTE_MEMSET(dest, value, size)                                    \
...
#define CARD_COMPARE(c1, c2) ((SUIT(c1) == SUIT(c2)) && (RANK(c1) >= RANK(c2)))
...
#define DECK_WINNING_CARD_IDX(d) (_deck_winning_card_index((d), (d##_len)))
...
#define BUSSO_KIND(b) ((b) & BUSSO_KIND_MASK)
...
#define BUSSO(kind, suit, rank)                                                \
  ((kind) | (((suit) << 2) | (BUSSO_ENCODE_RANK(rank))))
enum {
  NAPOLI = 0x10,
  BONGIOCO = 0x20,
  STELLA = 0x30,
...
#define STEP(step, p) (step | GET_PAYLOAD(p))
...
#define GAME_TABLE_ADD_CARD(g, c) ((g)->table[(g)->table_len++] = (c))
#define GAME_CURRENT_PLAYER(g) (&(g)->players[(g)->current_player])
#define GAME_NEXT_PLAYER(g) (((g)->current_player + 1) % ((g)->players_len))
#define GAME_ADVANCE_PLAYER(g) ((g)->current_player = GAME_NEXT_PLAYER(g))
#define GAME_TURN_END(g) ((g)->table_len == (g)->players_len)
...
static inline point_t card_points_classic(card_t c) { return CARD_POINTS(c); }
...
static inline void deck_init(card_t *d) {
  card_t *c = d;
  for (card_t s = BASTONI; s <= COPPE; s++)
    for (card_t r = QUATTRO; r <= TRE; r++)
      *c++ = CARD(s, r);
}
...
static inline void _game_score(struct game_t *g) {
  ...
  TRESSETTE_MEMCPY(p->deck + p->deck_len, g->table,
...
static inline message_t game_dispatch(struct game_t *g, action_t a) {
  // error not playing
  if (g->state != GAME_PLAYER_TURN)
    return MAKE_MESSAGE(ERROR, ERR_NOTPLAYING);
...
static inline message_t game_dispatch_auth(struct game_t *g, uint8_t idx,
                                           action_t a) {
...
static inline point_t reduce_deck_points(card_t *deck, int8_t deck_len,
                                         point_calc_t fn) {
  point_t acc = 0;
...
static inline point_calc_t
settings_get_point_calculator(game_settings_t settings) {
